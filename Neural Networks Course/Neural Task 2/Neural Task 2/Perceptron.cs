...
namespace Neural_Task_2
{
    class Perceptron
    {
        ...
        public Perceptron(double eta, int epochs, int c1, int c2,
            int f1, int f2)
        {
        ...
        public static void displayConfusionMatrix(ref DataGridView dataGridView, int[,] best_confusion_matrix)
        {
        ...
        public int[,] prepareConfusionMatrix()
        {
            for (int i = 1; i < 5; i++)
            {
                for (int j = 1; j < 4; j++)
        ...
        {
            Matrix<double> misx = Matrix<double>.Build.Dense(3, 1);
            for (int i = 0; i < epochs; i++)
            {
                ...
                weight = weight.Add(misx.Multiply(eta));
            }
        }
